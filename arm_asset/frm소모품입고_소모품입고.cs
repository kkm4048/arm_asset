...
namespace arm_asset
{
    public partial class frm소모품입고_소모품입고 : Form
    {
        string sql;
        bool r초기화 = true;
        TextBox rtb;
         public frm소모품입고_소모품입고()
        {
            InitializeComponent();
        }

        private void frm소모품입고_소모품입고_Load(object sender, EventArgs e)
        {
            string d;
        ...
        private void frm소모품입고_소모품입고_Activated(object sender, EventArgs e)
        {
            포커스(txt1);
        }
        ...
        private void 조회0()
        {
            ...
            {
                w = w + " and (a.d1 like '%" + txt조회.Text + "%'  or a.d2 like '%" + txt조회.Text + "%'  or a.d3 like '%" + txt조회.Text + "%'    or b.d1 like '%" + txt조회.Text + "%'   )  \n";
            }
        ...
        private void 조회1()
        {
            ...
            sql = "";
            sql = sql + " select a.dt ,a.tm,a.state,a.num1,a.num2,a.arm_code,b.arm ,a.code,a.usr,a.chk ,d.d1,d.d2,a.cnt,a.d1,a.d2,a.d3 ";
            ...
            sql = sql + "   and a.arm_code = '" + cls_com.GetCode(cmb소속부대.Text) + "' ";
            sql = sql + "   and a.state = '입고'  ";
        ...
        private void 헤더(DataSet ds)
        {
            ...
            spr.ActiveSheet.ColumnHeader.Cells[0, i++].Text = "입고수량";
        ...
        private void txt수량_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                저장(txt1.Text, txt수량.Text ,chk취소.Checked);
            }
        }
        ...
        private void 등록()
        {
            ...
            string 소속코드 = "";
            DataSet ds;
            if (cls_com.GetCode(cmb소속부대.Text).Equals("")) {
                
                MessageBox.Show("부대를 선택 하세요");
                return;
                
            }
            소속코드 = cls_com.GetCode(cmb소속부대.Text);
            ...
            ds = cls_com.Select_Query("select * from b201_stock  where code = '" + txt1.Text + "' and arm_code = '" + cls_com.GetCode(cmb소속부대.Text) + "' and d1 = '" + txt사업명.Text.Trim() + "' ");
            if (ds.Tables[0].Rows.Count > 0)
            {

                증빙번호 = cls_com.증빙번호_소모품_GET("입고",cls_com.GetDate(dtp.Value), 소속코드,"");
                ...
                tran = cls_com.gCon.BeginTransaction();
                    ...
                    sql = "UPDATE b201_stock set cnt = cnt + " + cls_com.Val2(txt수량.Text).ToString() + " where  code = '" + txt1.Text + "' and arm_code = '" + cls_com.GetCode(cmb소속부대.Text) + "' and d1 = '" + txt사업명.Text.Trim() + "' ";
                    cls_com.ExcuteNonQueryT(sql,tran);
                    sql = "insert into b301_output(dt,tm,num1,num2,state,arm_code,arm_code_s,code,usr,chk,cnt,d1,d2,d3,d4,d5) values  ('" 
                           +  cls_com.GetDate(dtp.Value) + "','" + cls_com.GetTime() + "','" + 증빙번호 + "','" + 증빙번호_번호 + "','" + "입고" + "','" + 소속코드 +  "','','" + txt1.Text + "','" 
                           +  cls_com.사용자.성명 + "','','" +  cls_com.Val2(txt수량.Text).ToString() + "','" + txt사업명.Text + "','" + txt예산구분.Text + "','" + txt근거.Text + "','" + txt비고.Text + "','" + txt출납관.Text +"' ) " ;
                    cls_com.ExcuteNonQueryT(sql,tran);
               //     sql = " update b301_output set d3 = '" + txt출납관.Text + "' where num1 = '" + 증빙번호 + "' ";
             //       cls_com.ExcuteNonQueryT(sql, tran);
                cls_com.로그(cls_com.사용자.성명, "소모품 입고", "입고", sql);
            ...
            else
            {
                ...
                sql = "insert into  b201_stock (code,dt,state,arm_code,cnt,d1,d2,d3) values ('" + txt1.Text + "','" + cls_com.GetDate(dtp.Value) + "','" + "" + "','"
                    + cls_com.GetCode(cmb소속부대.Text) + "','" + cls_com.Val2(txt수량.Text).ToString() + "','" + txt사업명.Text.Trim() + "','',''  ) ";
                cls_com.ExcuteNonQueryT(sql,tran);
                sql = "insert into b301_output(dt,tm,num1,num2,state,arm_code,arm_code_s,code,usr,chk,cnt,d1,d2,d3,d4,d5) values  ('"
                       + cls_com.GetDate(dtp.Value) + "','" + cls_com.GetTime() + "','" + 증빙번호 + "','" + 증빙번호_번호 + "','" + "입고" + "','" + 소속코드 + "','','" + txt1.Text + "','"
                       + cls_com.사용자.성명 + "','','" + cls_com.Val2(txt수량.Text).ToString() + "','" + txt사업명.Text + "','" + txt예산구분.Text + "','" +  txt근거.Text + "','" + txt비고.Text + "','" + txt출납관.Text + "' ) ";
        ...
        private void 수정()
        {
            ...
            sql = sql + "update b301_output set  ";
            sql = sql + "    d3 = '" + txt출납관.Text + "' ";
            sql = sql + "where num1 = '" + txt증빙번호.Text + "'   \n ";

            cls_com.ExcuteNonQuery(sql);
            cls_com.로그(cls_com.사용자.성명, "소모품 입고", "수정", sql);
            조회();
        ...
        private void 삭제()
        {
            ...
            DataSet ds;
            String 수량 = "0";
            if (cls_com.GetCode(cmb소속부대.Text).Equals("")) return;
            ...
            sql = "SELECT * FROM b201_stock where code = '" + txt1.Text + "' and arm_code = '" + cls_com.GetCode(cmb소속부대.Text) + "' ";
            ds = cls_com.Select_Query(sql);
            if (ds.Tables[0].Rows.Count > 0)
            {
                sql = "update b201_stock set cnt = cnt - " + 수량 + " where  code = '" + txt1.Text + "' and arm_code = '" + cls_com.GetCode(cmb소속부대.Text) + "' ";
            }
            else
            {
                sql = "insert into  b201_stock  (code,dt,state,arm_code,cnt,d1,d2,d3) values ('" + txt1.Text + "','" + cls_com.GetDate() + "','','" + cls_com.GetCode(cmb소속부대.Text) + "','" + 수량 + "','','','' ) ";
            }
            cls_com.ExcuteNonQuery(sql);

            cls_com.로그(cls_com.사용자.성명, "소모품 입고", "삭제", sql);
            조회();
        }
        ...
        private void spr_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            txt1.Text = spr.ActiveSheet.Cells[e.Row, 7].Text;
            txt2.Text = spr.ActiveSheet.Cells[e.Row, 11].Text;
            txt근거.Text = spr.ActiveSheet.Cells[e.Row, 13].Text;
            txt비고.Text = spr.ActiveSheet.Cells[e.Row, 14].Text;
            txt출납관.Text = spr.ActiveSheet.Cells[e.Row, 15].Text;
        ...
        private void btn출력_거래증_Click(object sender, EventArgs e)
        {
            ...
            부대1 = spr.ActiveSheet.Cells[0, 5].Text + " " + spr.ActiveSheet.Cells[0, 6].Text;
            부대2 = "";
            증빙번호 = txt증빙번호.Text;
            frm출력_거래증 frm출력_거래증 = new frm출력_거래증("소모품_입고", cls_com.GetDate(dtp.Value), "입고", 증빙번호, 부대1, 부대2);
