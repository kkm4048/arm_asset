...
namespace arm_asset
{
    public partial class frm자산불출_자산폐기매각 : Form
    {
        string sql;
        bool r초기화 = true;
        int r선택수량 = 0;
        public frm자산불출_자산폐기매각()
        {
            InitializeComponent();
        }

        private void frm자산불출_자산폐기매각_Load(object sender, EventArgs e)
        {
            string d;
        ...
        private void Add상태()
        {
            cmb상태.Items.Clear();
            cmb상태.Items.Add("");
//            cmb상태.Items.Add("매각대기");
            cmb상태.Items.Add("매각");
  //          cmb상태.Items.Add("폐기대기");
            cmb상태.Items.Add("폐기");
        ...
        private void 조회0()
        {
            ...
            if (!txt조회.Text.Equals("") ) {
                ...
                w = w + "  or  a.d1 like '%" + txt조회.Text + "%' ";
                w = w + "  or  a.d2 like '%" + txt조회.Text + "%' ";
                w = w + "  or  a.d3 like '%" + txt조회.Text + "%' ";
                w = w + "  or  a.d4 like '%" + txt조회.Text + "%' ";
                w = w + "  or  a.d5 like '%" + txt조회.Text + "%' ";
                w = w + "  or  a.d12 like '%" + txt조회.Text + "%' ";
                w = w + "  ) ";
        ...
        private void 조회1()
        {
            ...
            sql = sql + "   and ( a.state = '매각'  or a.state = '매각대기'  or a.state = '폐기' or a.state = '폐기대기' )   ";
        ...
        private void txt1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                저장(txt1.Text,chk취소.Checked,false);
            }
        }

        private void 저장(string 자산번호, bool 취소여부, bool 선택)
        {
            String 상태 = "", 재고여부 = "";
            String 증빙번호 = "", 증빙번호_번호 = "";
            String 소속코드, 소속명, 불출_소속코드, 불출_소속명;
            String 일자, 시간;
            String 상태_저장 = cmb상태.Text;
            자산번호 = 자산번호.ToUpper();
            ...
            if (취소여부)
            {
                ...
                else if (상태.Equals("폐기대기"))
                {
                    상태_저장 = "";
                }
                else if (상태.Equals("매각"))
                {
                    상태_저장 = "매각대기";
                }
                else if (상태.Equals("매각대기"))
                {
                    상태_저장 = "";
                }
                else
                {
                    MessageBox.Show("상태에러" + " " + 상태);
                    포커스();
                    return;
                ...
                sql = "";
                sql = "select * from  a301_output where arm_code_s = '" + cls_com.GetCode(cmb소속부대.Text) + "' and  dt = '" + 일자 + "' and code = '" + 자산번호 + "' and state ='" + 상태 + "' and chk = '' ";
                ...
                sql = "";
                sql = "delete from a301_output where arm_code_s = '" + cls_com.GetCode(cmb소속부대.Text) + "' and  dt = '" + 일자 + "' and code = '" + 자산번호 + "' and state ='" + 상태 + "' and chk = '' ";
                if (!cls_com.ExcuteNonQueryT(sql, tran).Equals("success"))
                {
                    tran.Rollback();
                    포커스();
                ...
                sql = " UPDATE a201_asset  set state = '" + 상태_저장 + "' where code = '" + 자산번호 + "' ";
                if (!cls_com.ExcuteNonQueryT(sql, tran).Equals("success"))
                {
                    tran.Rollback();
                    포커스();
                ...
                tran.Commit();
                cls_com.gCon.Close();
                cls_com.로그(cls_com.사용자.성명, "자산 " + cmb상태.Text, "취소", sql);
                int irow = -1, icol = -1;
                ...
                if ((cmb상태.Text.Equals("매각")) || cmb상태.Text.Equals("폐기"))
                {
                } else
                {
                    MessageBox.Show("상태를 확인하세요");
                    포커스();
                    return;
                }
                if (상태.Equals("매각"))
                {
                    MessageBox.Show("이미 매각한 자산입니다");
                    포커스();
                    return;
                }
                if (상태.Equals("폐기"))
                {
                    MessageBox.Show("이미 폐기한 자산입니다");
                    포커스();
                    return;
                }
                if (상태.Equals("매각대기"))
                ...
                증빙번호 = cls_com.증빙번호_자산_GET(상태_저장, cls_com.GetDate(dtp.Value), 소속코드, 불출_소속코드);
                ...
                tran.Commit();
                cls_com.gCon.Close();
                cls_com.로그(cls_com.사용자.성명, "자산 불출", 상태, sql);
                int irow = -1, icol = -1;

                spr0.Search(0, 자산번호, false, false, false, false, 0, 2, ref irow, ref icol);
                if (irow >= 0)
                {
                    spr0.ActiveSheet.Cells[irow, 1].Text = 상태;
                }
        ...
        private void 폐기(string 자산번호, bool 취소여부)
        {
            ...
            if (취소여부)
            {
                ...
                tran = cls_com.gCon.BeginTransaction();
                sql = "";
                sql = "select * from  a301_output where arm_code_s = '" + cls_com.GetCode(cmb소속부대.Text) + "' and  dt = '" + 일자 + "' and code = '" + 자산번호 + "' and state ='" + 상태_저장 + "' and chk = '' ";
                DataSet ds0 = cls_com.Select_Query(sql);
                if (ds0.Tables[0].Rows.Count > 0)
                {
                    sql = "update a301_output set num2 = num2 -1 where num1 = '" + ds0.Tables[0].Rows[0]["num1"].ToString() + "' and num2 > " + ds0.Tables[0].Rows[0]["num2"].ToString() + " ";
                ...
                sql = "";
                sql = "delete from a301_output where arm_code_s = '" + cls_com.GetCode(cmb소속부대.Text) + "' and  dt = '" + 일자 + "' and code = '" + 자산번호 + "' and state ='"  +  상태_저장 + "' and chk = '' ";
                if (!cls_com.ExcuteNonQueryT(sql, tran).Equals("success"))
                {
                    tran.Rollback();
                    return;
                ...
                tran.Commit();
                cls_com.gCon.Close();
                cls_com.로그(cls_com.사용자.성명, "자산 " + 상태_저장 , "취소", sql);
                int irow = -1, icol = -1;
            ...
            else
            {
                ...
                if (상태.Equals("매각대기") || 상태.Equals("폐기대기") || 상태.Equals("")) 
                {

                } else 
                {
                    MessageBox.Show("상태 : " + 상태 + "   불가");
                    포커스();
                    return;
                }
                증빙번호 = cls_com.증빙번호_자산_GET(상태_저장 , cls_com.GetDate(dtp.Value), 소속코드, 불출_소속코드);
                ...
                tran.Commit();
                cls_com.gCon.Close();
                cls_com.로그(cls_com.사용자.성명, "자산 " + 상태_저장, 상태_저장, sql);
                int irow = -1, icol = -1;

                spr0.Search(0, 자산번호, false, false, false, false, 0, 2, ref irow, ref icol);
                if (irow >= 0)
                {
                    spr0.ActiveSheet.Cells[irow, 1].Text = 상태_저장;
                }
           ...
           for (int i = 0; i < spr0.ActiveSheet.RowCount;i++)
            {
                ...
                {
                    저장(spr0.ActiveSheet.Cells[i, 2].Text,false ,true );
                }
        ...
        private void 선택불출취소()
        {
            ...
            for (int i = 0; i < spr0.ActiveSheet.RowCount; i++)
            {
                if (spr0.ActiveSheet.Cells[i, 0].Text.Equals("True"))
                {
                    저장(spr0.ActiveSheet.Cells[i, 2].Text,true,true);
                }
        ...
        private void btn출력_거래증_Click(object sender, EventArgs e)
        {
            ...
            string 증빙번호 = "", 부대1 = "", 부대2 = "",상태;
            int row = -1;
            if (spr.ActiveSheet.Rows.Count < 0) return;
            if (spr.ActiveSheet.ActiveRow.Index < 0) return;
            row = spr.ActiveSheet.ActiveRow.Index;
            부대1 = spr.ActiveSheet.Cells[row, 3].Text + " " + spr.ActiveSheet.Cells[row, 4].Text;
            부대2 = spr.ActiveSheet.Cells[row, 5].Text + " " + spr.ActiveSheet.Cells[row, 6].Text; ;
            증빙번호 = spr.ActiveSheet.Cells[row, 7].Text;
            상태 = spr.ActiveSheet.Cells[row, 2].Text;
            frm출력_거래증 frm출력_거래증 = new frm출력_거래증("자산", cls_com.GetDate(dtp.Value), 상태, 증빙번호, 부대1 ,부대2 );
        ...
        private void 선택폐기()
        {
            ...
            DialogResult dr = MessageBox.Show("선택 " + cmb상태.Text + " " + lbl선택수량.Text + " 건 " + cmb상태.Text  +"하시겠습니까?", "선택 " + cmb상태.Text, MessageBoxButtons.YesNo);
            if (dr == DialogResult.No) return;
        ...
        private void 선택폐기취소()
        {
            ...
            DialogResult dr = MessageBox.Show("선택 " + cmb상태.Text + " 취소  " + lbl선택수량.Text + " 건 " + cmb상태.Text  + " 취소 하시겠습니까?", "선택 " + cmb상태.Text  +" 취소", MessageBoxButtons.YesNo);
            if (dr == DialogResult.No) return;
