...
namespace arm_asset
{
    public partial class frm소모품불출수령_소모품불출수령조회 : Form
    {
        string sql;
        bool r초기화 = false;
        public frm소모품불출수령_소모품불출수령조회()
        {
            InitializeComponent();
        }

        private void frm소모품불출수령_소모품불출수령조회_Load(object sender, EventArgs e)
        {
            string d;
            초기화();
        ...
        private void frm소모품불출수령_소모품불출수령조회_Activated(object sender, EventArgs e)
        {
            txt1.Focus();
        }
        private void 조회0()
        ...
        private void btn출력_거래증_Click(object sender, EventArgs e)
        {
            ...
            {
                frm출력_거래증 frm출력_거래증 = new frm출력_거래증("소모품_소모처리", cls_com.GetDate(dtp.Value), txt상태.Text, txt증빙번호.Text, txt부대1.Text, txt부대2.Text);
                frm출력_거래증.ShowDialog();
            } else
            {
                frm출력_거래증 frm출력_거래증 = new frm출력_거래증("소모품", cls_com.GetDate(dtp.Value), txt상태.Text, txt증빙번호.Text, txt부대1.Text, txt부대2.Text);
                frm출력_거래증.ShowDialog();
        ...
        private void btn엑셀저장_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.FileName = "소모품불출수령_" + cls_com.GetDate();
